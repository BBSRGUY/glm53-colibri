...
- **Vision works** (`vision/`) — text, images and video. A PyTorch sidecar runs the 563.6M ViT
  ...
  real photographs, multiple images per turn and 8-frame video clips all work, including
  **motion** — but the question has to be decomposed: "which direction?" fails, while
  "where is it in Frame 1? in Frame 4?" is answered correctly and inverts for a reversed
  clip. See finding #15 and `vision/probe_motion_staged.py`. See finding #15: one missing GELU in the merger made
