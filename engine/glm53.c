/* GLM-5.3-Flash engine (architettura glm5_next) in C puro.
 *
 * Forked from colibri.c (GLM-5.2, glm_moe_dsa). What GLM-5.3 keeps unchanged, and
 * why this is a fork rather than a rewrite:
 *   - sigmoid router + noaux_tc (n_group=1) with routed_scaling_factor  -- identical
 *   - shared expert + routed experts streamed per-expert from disk     -- identical
 *   - first_k_dense_replace dense layers                               -- identical
 *   - MTP head (eh_proj/enorm/hnorm/shared_head.norm)                  -- identical names
 *   - DSA lightning indexer (wq_b / wk / weights_proj)                 -- identical names
 *
 * What is new in GLM-5.3, and where it comes from:
 *   - 34 of 45 layers are KDA (Kimi Delta Attention) linear attention. kda_forward()
 *     below is ported from kimi_k3.c:1167. Every tensor name matches Kimi's loader
 *     except the gate, which GLM-5.3 factors low-rank (g_a_proj/g_b_proj, rank 128)
 *     where Kimi has a dense g_proj -- handled exactly like the existing f_a/f_b pair.
 *   - mHC (manifold-constrained hyper-connections), hc_mult=4. The math is
 *     dsv4_mhc.h verbatim (DeepSeek-V4); GLM-5.3 ships the same six tensors
 *     (hc_{attn,ffn}_{fn,scale,base}) with the same [2M+M*M, M*H] layout.
 *     This is the invasive change: the residual stream becomes [S,M,D], not [S,D].
 *   - MLA has NO RoPE (mla_use_nope=true, qk_rope_head_dim=0), so the interleaved
 *     partial-RoPE path from GLM-5.2 is dead code here. Position lives in KDA.
 *   - Multimodal: tensors are nested under "model.language_model." and a 24-block
 *     ViT lives under "model.visual.". The ViT is NOT consumed here (see g_pfx).
 *
 * mHC RESIDUAL, resolved against the reference implementations:
 *   - SEED: the embedding is BROADCAST into all M streams
 *     (DeepseekV4Model.forward: inputs_embeds.unsqueeze(2).expand(-1,-1,hc_mult,-1);
 *     llama.cpp: reshape_3d + repeat_4d). Not stream-0-with-zeros.
 *   - COLLAPSE: arithmetic MEAN over the M streams. This is where GLM-5.3 DIVERGES
 *     from DeepSeek-V4, which applies a LEARNED collapse (DeepseekV4HyperHead, with
 *     its own hc_fn/hc_base/hc_scale). GLM-5.3's checkpoint contains NO model-level
 *     hyper-connection tensors -- only lm_head, embed_tokens and norm sit outside the
 *     layers -- and llama.cpp's glm5next graph collapses with glm5next_hc_mean (sum/hc).
 *   - comb is contracted TRANSPOSED (sum over the first stream axis). Sinkhorn yields
 *     a doubly-stochastic but non-symmetric matrix, so the axis matters; dsv4_mhc_post
 *     already indexes it that way.
 *   - post_mult = 2.0, and both epsilons are hc_eps (coli_v4_hc_split_sinkhorn).
 *
 * VISION (not consumed here; recorded so Phase 3/5 does not have to re-derive it):
 * the tower carries NO positional tensor -- no pos_embed, no learned APE, nothing
 * outside patch_embed / the 24 blocks / downsample / merger / post_layernorm. GLM-4V
 * combines a LEARNED position_embedding (bicubic-interpolated to the patch grid) WITH
 * 2D RoPE; GLM-5.3 dropped the learned half, which is exactly why the tensor is absent.
 * What remains is parameter-free 2D RoPE, transferable verbatim from Glm4vVisionRotary-
 * Embedding: inv_freq = 1/(10000^(arange(0,dim,2)/dim)), freqs = pos * inv_freq then
 * flatten, applied with the usual rotate_half; positions come from
 * get_vision_position_ids(grid_thw, spatial_merge_size), i.e. (h,w) coordinates emitted
 * in SPATIAL-MERGE-BLOCK order, not raster order. At 448px/patch 14 that is a 32x32 grid
 * -> 1024 patches -> 256 tokens after the 2x2 merge.
 *
 * DSA: the k-pool indexer is implemented (pool keys are a per-channel convex mix
 * softmax(gate + ape) . member keys, scored per pool, tail always visible). Verified
 * cell-set-identical to a numpy reference on an all-f32 container; under quantization
 * the selection drifts because the indexer INPUT drifts, not the indexer itself.
 *
 * STATUS: builds and parses the real config; the layer math is wired end to end.
 * NOT validated numerically -- never run against real weights, never compared to
 * transformers. See COLI_GLM53_TODO markers.
 */
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
...
#endif
#include "st.h"
/* GLM-5.3's mHC is DeepSeek-V4's, unchanged: same six tensors, same
 * [2M+M*M, M*H] layout, same Sinkhorn. Included rather than reimplemented so the
 * two engines cannot drift. (llama.cpp's glm5next PR reaches the same conclusion --
 * its hyper-connection class inherits from DeepseekV4HyperConnection with a bare pass.)
 * The three constants dsv4_mhc.h takes as parameters are pinned by
 * coli_v4_hc_split_sinkhorn in deepseek_v4.c:1494: post is scaled by 2.0, and both
 * the pre and Sinkhorn epsilons are hc_eps. */
#include "glm53_mhc.h"   /* pulls in dsv4_mhc.h */
#ifdef __linux__
#include "uring.h"
#endif
#include "tok.h"
...
/* GLM-5.3 nests the text stack under "model.language_model.". Every tensor-name
 * macro below interpolates this between "model." and "layers.%d", so an empty
 * string reproduces GLM-5.2 naming exactly. Set by load_cfg() from model_type. */
static const char *g_pfx = "";
/* GLM-5.3 clamped-SwiGLU bound. A global because expert_ffn() runs deep under moe()
 * and has no Cfg in scope; set once in load_cfg, read-only afterwards. 0 = unclamped
 * (GLM-5.2, whose config has no swiglu_limit). */
static float g_swiglu_limit = 0.f;
/* COLI_GLM53_NO_MHC=1: proceed without the mHC residual (bring-up escape hatch). */
static int g_glm53_allow_no_mhc=0;
/* COLI_GLM53_KDA_SPEC=1: allow speculative decoding even though KDA state
 * cannot be rolled back on a rejected draft. Diagnostic only. */
static int g_glm53_kda_spec=0;
/* Built once in load_cfg() so the embed / final-norm lookups pick up the same nest. */
static char g_embed_name[64]="model.embed_tokens.weight";
static char g_fnorm_name[64]="model.norm.weight";
#define EMBED_NAME g_embed_name
#define FNORM_NAME g_fnorm_name
...
typedef struct {
    ...
    /* --- GLM-5.3 --- */
    int8_t is_kda[128];       /* per layer: 1 = KDA linear attention, 0 = MLA/DSA full attention */
    int kda_heads, kda_hd, kda_proj, kda_rank, conv_k;   /* proj = heads*hd; rank = f/g LoRA rank */
    float gate_lb;            /* linear_attn_config.gate_lower_bound (-5.0) */
    int hc_mult, hc_sinkhorn_iters;              /* mHC: M, Sinkhorn iterations */
    float hc_eps;
    int mla_nope;             /* mla_use_nope: 1 => no RoPE anywhere in attention */
    int index_kpool, index_kpool_compress, index_kpool_tail;   /* GLM-5.3 k-pool DSA */
    float swiglu_limit;       /* GLM-5.3: clamped SwiGLU bound (0 = unclamped) */
} Cfg;
...
typedef struct {
    ...
    /* --- GLM-5.3: KDA linear-attention layer (used when c->is_kda[li]) ---
     * Names and shapes match kimi_k3.c's Kda exactly, with one addition: GLM-5.3
     * factors the output gate low-rank (ga/gb, rank = kda_hd) where Kimi has a single
     * dense g_proj. Everything else -- conv taps, decay LoRA, beta proj, dt_bias,
     * exp(A_log), o_norm -- is shape-for-shape identical (verified against the real
     * checkpoint headers, layer 0). */
    QT kq, kk, kv, ko;                     /* q/k/v_proj [P,D] and o_proj [D,P] */
    float *conv_q, *conv_k, *conv_v;       /* [P*conv_k] depthwise taps, oldest first */
    float *fa, *fb;                        /* decay LoRA: [kda_hd,D] and [P,kda_hd] */
    float *ga, *gb;                        /* gate LoRA: [kda_hd,D] and [P,kda_hd] */
    float *bp;                             /* beta proj [heads,D] */
    float *dt, *A, *onw;                   /* dt_bias[P], exp(A_log)[heads], o_norm[kda_hd] */
    /* --- GLM-5.3: mHC (hyper-connections). fn is [2M+M*M, M*D], base [2M+M*M], scale[3] --- */
...
typedef struct {
    ...
    /* --- GLM-5.3 KDA state. Both are CONSTANT in context length, unlike the MLA KV
     * cache: kstate[li] is [heads*hd*hd] (4 MB/layer at 64x128x128) and the three conv
     * windows are [proj*conv_k]. Allocated only for layers where c->is_kda[li]. --- */
    ...
    /* GLM-5.3 k-pool: per-layer compressor gate projection [index_hd, hidden] and the
     * intra-pool position bias [index_kpool, index_hd]. Gc mirrors Ic and holds the gate
     * logits of every cached token -- they depend on the token hidden state, so they
     * cannot be recomputed from the key cache later. */
    QT *ix_gate; float **ix_ape; float **Gc;
...
static void expert_ffn(float *hh, float *gg, float *uu, const float *xg, QT *g, QT *u, QT *d, int nr, int I){
    ...
    if(g_swiglu_limit>0.f){                      /* GLM-5.3 clamped SwiGLU (see dense_mlp) */
        float L=g_swiglu_limit;
        #pragma omp parallel for schedule(static) if((int64_t)nr*I>=16384)
        for(int64_t z=0;z<(int64_t)nr*I;z++){
            float g1=gg[z]>L?L:gg[z];
            float u1=uu[z]>L?L:(uu[z]<-L?-L:uu[z]);
            gg[z]=siluf(g1)*u1;
        }
    } else
    #pragma omp parallel for schedule(static) if((int64_t)nr*I>=16384)
...
static void load_cfg(Cfg *c, const char *snap){
    ...
    /* GLM-5.3-Flash is natively multimodal: model_type "glm5_next" at the root, with the
     * entire text stack (dims, experts, attention) under "text_config" and a ViT under
     * "vision_config". GLM-5.2 kept everything at the root. Descend when the nested
     * section exists so one engine reads both, and remember the tensor-name prefix that
     * goes with the nested layout. */
    jval *r=root; jval *tc=json_get(root,"text_config");
    if(tc && tc->t==J_OBJ){
        r=tc; g_pfx="language_model.";
        snprintf(g_embed_name,sizeof(g_embed_name),"model.%sembed_tokens.weight",g_pfx);
        snprintf(g_fnorm_name,sizeof(g_fnorm_name),"model.%snorm.weight",g_pfx);
    }
    ...
    /* GLM-5.3 k-pool indexer (absent in GLM-5.2, unimplemented here -- see glm53_kpool_guard) */
    c->index_kpool=gi(r,"index_kpool");
    { jval *kc=json_get(r,"index_kpool_compress");
      c->index_kpool_compress=(kc&&kc->t==J_BOOL)?kc->boolean:0;
      jval *kt=json_get(r,"index_kpool_always_select_tail");
      c->index_kpool_tail=(kt&&kt->t==J_BOOL)?kt->boolean:0; }
    ...
    /* ---- GLM-5.3: hybrid attention stack, mHC, and the KDA geometry ---- */
    { jval *mn=json_get(r,"mla_use_nope"); c->mla_nope=(mn&&mn->t==J_BOOL)?mn->boolean:0; }
    { jval *sl=json_get(r,"swiglu_limit"); c->swiglu_limit=sl?(float)sl->num:0.f;
      if(c->swiglu_limit<0.f) c->swiglu_limit=0.f;
      g_swiglu_limit=c->swiglu_limit; }
    { jval *lt=json_get(r,"layer_types");
      for(int i=0;i<c->n_layers && i<128;i++) c->is_kda[i]=0;
      if(lt && lt->t==J_ARR)
          for(int i=0;i<c->n_layers && i<128 && i<lt->len;i++)
              c->is_kda[i] = (lt->kids[i]->str && !strcmp(lt->kids[i]->str,"linear_attention"));
    }
    { jval *la=json_get(r,"linear_attn_config");
      if(la && la->t==J_OBJ){
          c->kda_heads=gi(la,"num_heads"); c->kda_hd=gi(la,"head_dim");
          c->conv_k=gi(la,"short_conv_kernel_size");
          jval *gl=json_get(la,"gate_lower_bound"); c->gate_lb=gl?(float)gl->num:-5.f;
          c->kda_proj=c->kda_heads*c->kda_hd;
          /* f_a_proj is [head_dim, hidden] and f_b_proj is [proj, head_dim]; the low-rank
           * bottleneck IS head_dim. Same for the g_a/g_b gate pair GLM-5.3 adds. */
          c->kda_rank=c->kda_hd;
          /* kda_forward() keeps per-head scratch (qn/kn/alpha/kS/vt/oh) as fixed 512-float
           * stack arrays, inherited from kimi_k3.c. GLM-5.3 uses head_dim 128, but refuse
           * loudly rather than smash the stack if a future config exceeds that. */
          if(c->kda_hd>512){ fprintf(stderr,"kda head_dim=%d exceeds the 512 scratch bound\n",c->kda_hd); exit(1); }
          if(c->conv_k<1||c->conv_k>8){ fprintf(stderr,"short_conv_kernel_size=%d out of range\n",c->conv_k); exit(1); }
      } else {
          int nk=0; for(int i=0;i<c->n_layers && i<128;i++) nk+=c->is_kda[i];
          if(nk){ fprintf(stderr,"config: layer_types names %d linear_attention layers but linear_attn_config is missing\n",nk); exit(1); }
      }
    }
    { jval *mh=json_get(r,"mhc"); int on=(mh&&mh->t==J_BOOL)?mh->boolean:0;
      c->hc_mult = on ? gi(r,"hc_mult") : 1;
      if(c->hc_mult<1) c->hc_mult=1;
      c->hc_sinkhorn_iters=gi(r,"hc_sinkhorn_iters");
      jval *he=json_get(r,"hc_eps"); c->hc_eps=he?(float)he->num:1e-6f;
      /* dsv4_mhc.h sizes its mix[] scratch for 2M+M^2 <= 288, i.e. M <= 16. */
      if(c->hc_mult>16){ fprintf(stderr,"hc_mult=%d exceeds 16 (mHC scratch bound)\n",c->hc_mult); exit(1); }
    }
    /* indexer_types is per-layer but says nothing about attention type: GLM-5.3 marks all
     * 45 entries "full" while only the non-KDA blocks carry indexer weights. Leaving
     * idx_type set on a KDA layer makes the DSA auto-probe fail there and disable the
     * indexer for the entire model. */
    { int cleared=0;
      for(int i=0;i<c->n_layers && i<128;i++)
          if(c->is_kda[i] && c->idx_type[i]){ c->idx_type[i]=0; cleared++; }
      if(cleared) fprintf(stderr,"[cfg] indexer disabled on %d KDA layers (linear attention "
                                 "has no DSA indexer)\n", cleared); }
    ...
    /* GLM-5.3 sets mla_use_nope=true and qk_rope_head_dim=0 -- there is no RoPE anywhere
     * in the model, position lives entirely in the KDA recurrence. The GLM-5.2 bound of 1
     * would reject that config outright. */
    CKR("qk_nope_head_dim",c->qk_nope,1,1<<16)   CKR("qk_rope_head_dim",c->qk_rope,0,1<<16)
    ...
    #undef CKR
    { const char *e=getenv("COLI_GLM53_NO_MHC"); g_glm53_allow_no_mhc = e && atoi(e); }
    /* COLI_GLM53_KDA_SPEC=1 re-enables speculation despite the KDA state (for measuring
     * the drift, not for use): see glm53_spec_guard(). */
    { const char *e=getenv("COLI_GLM53_KDA_SPEC"); g_glm53_kda_spec = e && atoi(e); }
    { int nk=0; for(int i=0;i<c->n_layers && i<128;i++) nk+=c->is_kda[i];
...
/* Load at the width the CONTAINER carries: a tensor with a .qs sidecar keeps its
 * quantized format, one without is read as plain f32 (qt_alloc maps bits>=16 to fmt=0).
 * Used for the DSA indexer, which --indexer-f32 may leave unquantized. */
static QT qt_load_auto(Model *m, const char *name, int O, int I, int bits){
    char sn[300]; snprintf(sn,sizeof(sn),"%s.qs",name);
    return qt_load(m,name,O,I, st_has(&m->S,sn) ? bits : 16);
}
static QT qt_load(Model *m, const char *name, int O, int I, int bits){
...
/* ld() with an expected element count. A KDA layer loads eleven small f32 tensors whose
 * shapes encode the head geometry; if linear_attn_config and the checkpoint ever disagree
 * the mismatch must fail here, not silently misread the recurrence. */
static float *ldn(Model *m, const char *name, int64_t want){
    ...
    float *p=(float*)qalloc((size_t)n*sizeof(float));
...
static void model_init(Model *m, const char *snap, int cap, int ebits, int dbits){
    ...
    /* GLM-5.3 KDA state, allocated per KDA layer (NULL elsewhere so a stray deref traps). */
    m->kstate=calloc(NR,sizeof(float*)); m->cwq=calloc(NR,sizeof(float*));
    m->cwk=calloc(NR,sizeof(float*));    m->cwv=calloc(NR,sizeof(float*));
    for(int i=0;i<c->n_layers;i++){
        if(!c->is_kda[i]) continue;
        m->kstate[i]=calloc((size_t)c->kda_heads*c->kda_hd*c->kda_hd,sizeof(float));
        m->cwq[i]=calloc((size_t)c->kda_proj*c->conv_k,sizeof(float));
        m->cwk[i]=calloc((size_t)c->kda_proj*c->conv_k,sizeof(float));
        m->cwv[i]=calloc((size_t)c->kda_proj*c->conv_k,sizeof(float));
        if(!m->kstate[i]||!m->cwq[i]||!m->cwk[i]||!m->cwv[i]){
    ...
    for(int i=0;i<c->n_layers;i++){
        Layer *l=&m->L[i];
        #define P(s) (snprintf(nm,sizeof(nm),"model.%slayers.%d." s,g_pfx,i),nm)
        l->in_ln=ld(m,P("input_layernorm.weight"));
        l->post_ln=ld(m,P("post_attention_layernorm.weight"));
        /* --- GLM-5.3 mHC: six tensors per layer, both branches. Kept f32: hc_*_fn is
         * [2M+M*M, M*D] = 24x16384 here (1.5 MB), and the whole set across 45 layers is
         * ~141 MB. It sits on the residual path of EVERY layer, so it is the last thing
         * worth quantizing. Absent => hc_mult==1 and the classic single-stream residual. */
        if(c->hc_mult>1){
            int N=2*c->hc_mult+c->hc_mult*c->hc_mult;
            l->hc_attn_fn   = ldn(m,P("hc_attn_fn"),   (int64_t)N*c->hc_mult*D);
            l->hc_attn_base = ldn(m,P("hc_attn_base"), N);
            l->hc_attn_scale= ldn(m,P("hc_attn_scale"),3);
            l->hc_ffn_fn    = ldn(m,P("hc_ffn_fn"),    (int64_t)N*c->hc_mult*D);
            l->hc_ffn_base  = ldn(m,P("hc_ffn_base"),  N);
            l->hc_ffn_scale = ldn(m,P("hc_ffn_scale"), 3);
        }
        /* --- GLM-5.3: KDA layers carry no MLA tensors at all (no q_a/q_b/kv_*), so the
         * MLA loads below must not run for them -- st_die_missing would abort on the
         * first absent name. Tensor names and shapes follow kimi_k3.c:857-871. --- */
        if(c->is_kda[i]){
            int P_=c->kda_proj, R=c->kda_rank;
            l->kq = qt_load(m,P("self_attn.q_proj.weight"), P_, D, dbits);
            l->kk = qt_load(m,P("self_attn.k_proj.weight"), P_, D, dbits);
            l->kv = qt_load(m,P("self_attn.v_proj.weight"), P_, D, dbits);
            l->ko = qt_load(m,P("self_attn.o_proj.weight"), D, P_, dbits);
            l->conv_q = ldn(m,P("self_attn.q_conv1d.weight"), (int64_t)P_*c->conv_k);
            l->conv_k = ldn(m,P("self_attn.k_conv1d.weight"), (int64_t)P_*c->conv_k);
            l->conv_v = ldn(m,P("self_attn.v_conv1d.weight"), (int64_t)P_*c->conv_k);
            l->fa = ldn(m,P("self_attn.f_a_proj.weight"), (int64_t)R*D);
            l->fb = ldn(m,P("self_attn.f_b_proj.weight"), (int64_t)P_*R);
            /* The one shape difference from Kimi: the output gate is factored through the
             * same rank as the decay LoRA instead of being a dense [P,D] g_proj. */
            l->ga = ldn(m,P("self_attn.g_a_proj.weight"), (int64_t)R*D);
            l->gb = ldn(m,P("self_attn.g_b_proj.weight"), (int64_t)P_*R);
            l->bp = ldn(m,P("self_attn.b_proj.weight"), (int64_t)c->kda_heads*D);
            l->dt = ldn(m,P("self_attn.dt_bias"), P_);
            l->onw= ldn(m,P("self_attn.o_norm.weight"), c->kda_hd);
            /* A_log is stored as a log; the recurrence wants exp(A_log) per head. */
            l->A  = ldn(m,P("self_attn.A_log"), c->kda_heads);
            for(int h=0;h<c->kda_heads;h++) l->A[h]=expf(l->A[h]);
            goto mlp_load;                       /* skip the MLA block entirely */
        }
...
mlp_load:
    ...
    /* testa MTP (layer n_layers): presente solo se convertita con --mtp */
    {
        ...
        for(unsigned q=0;q<sizeof(req)/sizeof(req[0]);q++){
            snprintf(mn,sizeof(mn),"model.%slayers.%d.%s",g_pfx,c->n_layers,req[q]);
        ...
        snprintf(mn,sizeof(mn),"model.%slayers.%d.mlp.experts.%d.down_proj.weight",g_pfx,c->n_layers,c->n_experts-1);
        if(!st_has(&m->S,mn)) m->has_mtp=0;
        if(getenv("MTP") && atoi(getenv("MTP"))==0) m->has_mtp=0;
        if(m->has_mtp){
            int i=c->n_layers; Layer *l=&m->mtpL;
            #define PM(s) (snprintf(nm,sizeof(nm),"model.%slayers.%d." s,g_pfx,i),nm)
     ...
     * TUTTI i layer full. Auto-rilevamento come per MTP: niente flag, niente passi extra. */
    {
        ...
        for(int i=0;i<c->n_layers && m->has_dsa;i++){
            ...
            snprintf(inm,sizeof(inm),"model.%slayers.%d.self_attn.indexer.wq_b.weight",g_pfx,i);
        ...
        if(m->has_dsa){
            ...
            m->ix_gate=calloc(c->n_layers,sizeof(QT)); m->ix_ape=calloc(c->n_layers,sizeof(float*));
            for(int i=0;i<c->n_layers;i++){
                if(!c->idx_type[i]) continue;
                #define PI(s) (snprintf(nm,sizeof(nm),"model.%slayers.%d.self_attn.indexer." s,g_pfx,i),nm)
                m->ix_wq[i]=qt_load_auto(m,PI("wq_b.weight"), c->index_nh*c->index_hd, c->q_lora, dbits);
                m->ix_wk[i]=qt_load_auto(m,PI("wk.weight"), c->index_hd, D, dbits);
                m->ix_wp[i]=qt_load_auto(m,PI("weights_proj.weight"), c->index_nh, D, dbits);
                if(c->index_kpool_compress && c->index_kpool>1){
                    m->ix_gate[i]=qt_load_auto(m,PI("index_kpool_compress_gate"), c->index_hd, D, dbits);
                    m->ix_ape[i] =ldn(m,PI("index_kpool_compress_ape"), (int64_t)c->index_kpool*c->index_hd);
                }
...
static int expert_load_impl(Model *m, int layer, int eid, ESlot *s, int fatal, int demand){
    ...
    for(int k=0;k<3;k++) snprintf(nm[k],sizeof(nm[k]),"model.%slayers.%d.mlp.experts.%d.%s.weight",g_pfx,layer,eid,suf[k]);
...
static int worker_expert_missing_tensor(Model *m,int layer,int eid,char *out,size_t outsz){
    ...
    char nm[288], qn[320];
    snprintf(nm,sizeof(nm),"model.%slayers.%d.mlp.experts.%d.gate_proj.weight",g_pfx,layer,eid);
    snprintf(qn,sizeof(qn),"%s.qs",nm);
    if(!st_has(&m->S,qn)){                       /* unquantized: full weights only */
        for(int k=0;k<3;k++){
            snprintf(nm,sizeof(nm),"model.%slayers.%d.mlp.experts.%d.%s.weight",g_pfx,layer,eid,suf[k]);
...
static void expert_prefetch(Model *m, int layer, int eid){
    ...
    for(int k=0;k<3;k++){
        snprintf(nm,sizeof(nm),"model.%slayers.%d.mlp.experts.%d.%s",g_pfx,layer,eid,suf[k]);
...
static void attention_rows(Model *m, Layer *l, int layer, float *x, int S, int pos_base,
    ...
    /* Row stride into dsa_sel. k-pool keeps dtopk + r - 1 cells per row, so this is
     * NOT dtopk there; using dtopk overruns each row into the next. */
    int dsel_stride=0;
    if(m->has_dsa && layer<c->n_layers && ((!kvs && m->kv_start[layer]==0) || kvs)){
        int nh=c->index_nh, hd=c->index_hd; dtopk=c->index_topk;
        int kpool = (c->index_kpool_compress && c->index_kpool>1) ? c->index_kpool : 0;
        dsel_stride = dtopk + (kpool ? kpool-1 : 0);
        if(c->idx_type[layer]){
            ...
            for(int s=0;s<S;s++){
                ...
                /* GLM-5.3 is nope-only and llama.cpp's glm5next graph applies no rope to the
                 * indexer either -- the intra-pool ape carries the ordering. config.json still
                 * has indexer_rope_interleave=true; the running reference wins. */
                if(!kpool) rope_interleave(kd, pos, c);      /* primi qk_rope dim, interleaved */
            }
            if(kpool && m->Gc && m->Gc[layer]){              /* gate logits: cache per token */
                float *GD=falloc((int64_t)S*hd);
                matmul_qt_ex(GD, x, &m->ix_gate[layer], S, 0);
                for(int s=0;s<S;s++){
                    KVState *ks=kvs?kvs[s]:m->kv;
                    int pos=positions?positions[s]:pos_base+s;
                    memcpy(coli_kv_row(ks->Gc?ks->Gc[layer]:m->Gc[layer],pos,hd),
                           GD+(int64_t)s*hd, (size_t)hd*sizeof(float));
                }
                free(GD);
            }
            free(KD);
            if((int64_t)S*dsel_stride > m->dsa_scap){
                free(m->dsa_sel); free(m->dsa_nsel);
                m->dsa_scap=(int64_t)S*dsel_stride;
            ...
            /* k-pool: pool keys are query-independent, so build them once for the widest
             * context this call will score rather than once per row. */
            float *POOL=NULL; int npool_max=0;
            if(kpool){
                int nkmax=0;
                for(int s=0;s<S;s++){
                    int pos=positions?positions[s]:pos_base+s;
                    if(pos+1>nkmax) nkmax=pos+1;
                }
                npool_max = nkmax / kpool;
                if(npool_max>0){
                    KVState *ks0=kvs?kvs[0]:m->kv;
                    const float *IC=ks0->Ic[layer], *GC=ks0->Gc?ks0->Gc[layer]:m->Gc[layer];
                    const float *ape=m->ix_ape[layer];
                    POOL=falloc((int64_t)npool_max*hd);
                    #pragma omp parallel for schedule(static) if(npool_max>4)
                    for(int p=0;p<npool_max;p++){
                        float *pk=POOL+(int64_t)p*hd;
                        for(int ch=0;ch<hd;ch++){
                            float mx=-INFINITY, acc=0.f, den=0.f;
                            for(int mm=0;mm<kpool;mm++){
                                float v=coli_kv_row((float*)GC,p*kpool+mm,hd)[ch]+ape[(int64_t)mm*hd+ch];
                                if(v>mx) mx=v;
                            }
                            for(int mm=0;mm<kpool;mm++){
                                float v=coli_kv_row((float*)GC,p*kpool+mm,hd)[ch]+ape[(int64_t)mm*hd+ch];
                                float e=expf(v-mx);
                                den+=e; acc+=e*coli_kv_row((float*)IC,p*kpool+mm,hd)[ch];
                            }
                            pk[ch]=acc/(den>0.f?den:1e-20f);
                        if(p==2 && getenv("DSA_DUMP") && atoi(getenv("DSA_DUMP"))>=3){
                            fprintf(stderr,"[DSA_IN] L%d pool2 ch0 gate:",layer);
                            for(int mm=0;mm<kpool;mm++) fprintf(stderr," %.8f",coli_kv_row((float*)GC,2*kpool+mm,hd)[0]);
                            fprintf(stderr," | key:");
                            for(int mm=0;mm<kpool;mm++) fprintf(stderr," %.8f",coli_kv_row((float*)IC,2*kpool+mm,hd)[0]);
                            fprintf(stderr," | pk0=%.8f\n",pk[0]);
                        }
                        }
                    }
                }
            ...
            for(int s=0;s<S;s++){
                ...
                /* llama.cpp keeps index_topk + r - 1 cells: enough room for the
                 * incomplete tail on top of a whole number of pools. */
                int want = kpool ? dtopk + kpool - 1 : dtopk;
                int keep = nk<want ? nk : want;
                ...
                float *isc=falloc(nk);
                if(kpool){
                    /* score POOLS, then give every cell of a pool its pool's score. The r
                     * members tie, so the cut still lands on a pool boundary. Cells past the
                     * last whole pool are the incomplete tail: always visible. */
                    int np = nk / kpool, tail0 = np*kpool;
                    for(int p=0;p<np;p++){
                        const float *pk=POOL+(int64_t)p*hd;
                        float a=0;
                        for(int h=0;h<nh;h++){ const float *qhp=qi+(int64_t)h*hd;
                            float d0=0; for(int i=0;i<hd;i++) d0+=qhp[i]*pk[i];
                            d0*=rs; if(d0>0) a+=w32[h]*d0;
                        }
                        float sc=a*wsc;
                        for(int mm=0;mm<kpool;mm++) isc[p*kpool+mm]=sc;
                    }
                    if(getenv("DSA_DUMP") && atoi(getenv("DSA_DUMP"))>=2){
                        fprintf(stderr,"[DSA_PS] L%d row%d np=%d:",layer,pos,np);
                        for(int p2=0;p2<np;p2++)
                            fprintf(stderr," %.8f",isc[p2*kpool]);
                        fprintf(stderr,"\n");
                    }
                    for(int t=tail0;t<nk;t++) isc[t]= c->index_kpool_tail ? INFINITY : 0.f;
                } else
                for(int t=0;t<nk;t++){
                ...
                int *dst=m->dsa_sel+(int64_t)s*dsel_stride, nd=0;
                for(int t=0;t<nk && nd<keep;t++) if(isc[t]>thr) dst[nd++]=t;
                for(int t=0;t<nk && nd<keep;t++) if(isc[t]==thr) dst[nd++]=t;
                m->dsa_nsel[s]=nd;
                if(getenv("DSA_DUMP")){
                    /* selection only: sorted cell ids for this (layer,row) */
                    fprintf(stderr,"[DSA_SEL] L%d row%d n=%d:",layer,pos,m->dsa_nsel[s]);
                    for(int z=0;z<m->dsa_nsel[s];z++)
                        fprintf(stderr," %d",m->dsa_sel[(int64_t)s*dsel_stride+z]);
                    fprintf(stderr,"\n");
                }
                free(qi); free(w32); free(isc);
 free(tmp);
            }
        }
    ...
    if(absorb && c->kv_lora<=512){
           ...
           l->kv_b.cuda_eligible&&qt_cuda_upload(&l->kv_b)){   /* quant guard: absorb/kvdev read f32 Lc/Rc rows */
            ...
            for(int s=0;s<S&&cuda_core;s++){
                ...
                if(dnsel&&dnsel[s]>0){
                    ...
                    int ns=dnsel[s]; const int *tl=dsel+(int64_t)s*dsel_stride;
        ...
        for(int s=0;s<S;s++) for(int h=0;h<H;h++){
            ...
            const int *tlist = ns ? dsel+(int64_t)s*dsel_stride : NULL;
    ...
    if(dsa_any && flash_mb>0 && kvb_need>flash_mb*1048576){
        ...
        if(want){
            for(int s=0;s<S;s++){
                int ns=dnsel[s];
                if(ns>0){ const int *tl=dsel+(int64_t)s*dsel_stride;
...
static void moe(Model *m, Layer *l, int layer, float *x, int S, float *out, int with_shared){
    ...
    if(!shared_cuda){
        ...
        /* GLM-5.3 clamped SwiGLU, same asymmetry as dense_mlp/expert_ffn: the gate is
         * bounded above only, the up projection on both sides. */
        if(g_swiglu_limit>0.f){
            float L=g_swiglu_limit;
            for(int64_t z=0;z<(int64_t)S*sI;z++){
                float g1=sg[z]>L?L:sg[z];
                float u1=su[z]>L?L:(su[z]<-L?-L:su[z]);
                sg[z]=siluf(g1)*u1;
            }
        } else
...
static void dense_mlp(Layer *l, float *x, int S, int D, int I, float *out){
    ...
    /* GLM-5.3 clamped SwiGLU: gate is bounded ABOVE only, up on BOTH sides. */
    if(g_swiglu_limit>0.f){
        float L=g_swiglu_limit;
        for(int64_t i=0;i<(int64_t)S*I;i++){
            float gg=g[i]>L?L:g[i];
            float uu=u[i]>L?L:(u[i]<-L?-L:u[i]);
            g[i]=siluf(gg)*uu;
        }
    } else
...
/* ==================== GLM-5.3: KDA linear attention ====================
 * Ported from kimi_k3.c:1167 (kda_forward), with the Metal fast path and the
 * debug dumps dropped and the gate switched to GLM-5.3's low-rank ga/gb pair.
 *
 * Every dimension is config-driven (kda_proj/heads/hd/conv_k/gate_lb), exactly as
 * in Kimi -- no head count or head_dim is baked in. The only fixed sizes are the
 * per-head scratch arrays below, which is why load_cfg refuses kda_hd > 512.
 *
 * State: unlike MLA there is no growing KV cache. Each KDA layer carries a fixed
 * [heads*hd*hd] recurrent state plus three [proj*conv_k] convolution windows, both
 * independent of context length. That is what makes 1M context affordable here.
 *
 * COLI_GLM53_TODO: this has NOT been validated against transformers. The recurrence
 * is stateful across calls, so it is only correct when tokens are fed in order and
 * the state is reset between sequences (see kda_state_reset).
 */
/* ==================== GLM-5.3: mHC (hyper-connections) ====================
 * The residual stream is [S,M,D] with M = hc_mult (4). For each row: reduce the M
 * streams to one [D] layer input, run attention or the MLP on it, then expand the
 * result back across the M streams with the Sinkhorn-normalised mixing matrix.
 *
 * COLI_GLM53_TODO -- NOT YET ON THE FORWARD PATH. Two conventions still have to be
 * read out of the reference implementation before this can be switched on:
 *   1. INIT: how the [S,D] embedding seeds the M streams (broadcast to all M, or
 *      stream 0 only with the rest zeroed).
 *   2. FINAL: how the M streams collapse before the final norm and lm_head.
 * Guessing either would produce a model that runs and is quietly wrong, which is
 * worse than one that refuses. Until then load_cfg reports hc_mult and the engine
 * runs the classic single-stream residual below, which is exact only for hc_mult==1.
 */
static void mhc_block_pre(const Cfg *c, const float *residual, const float *fn,
                          const float *scale, const float *base,
                          float *post, float *comb, float *layer_input){
    glm53_mhc_pre(residual, fn, scale, base, c->hc_mult, c->hidden,
                  c->eps, c->hc_eps, c->hc_sinkhorn_iters, post, comb, layer_input);
}
static void mhc_block_post(const Cfg *c, const float *blk_out, const float *residual,
                           const float *post, const float *comb, float *out){
    glm53_mhc_post(blk_out, residual, post, comb, c->hc_mult, c->hidden, out);
}

static void f32_matmul(float *out, const float *in, const float *w, int C, int I, int O){
    #pragma omp parallel for schedule(static) if(C*O>4096)
    for(int t=0;t<C*O;t++){
        int cc=t/O, o=t%O; const float *a=in+(int64_t)cc*I, *b=w+(int64_t)o*I;
        float acc=0; for(int i=0;i<I;i++) acc+=a[i]*b[i];
        out[t]=acc;
    }
}

static void kda_state_reset(Model *m){
    Cfg *c=&m->c;
    if(!m->kstate) return;
    for(int i=0;i<c->n_layers;i++){
        if(!c->is_kda[i]) continue;
        if(m->kstate[i]) memset(m->kstate[i],0,(size_t)c->kda_heads*c->kda_hd*c->kda_hd*sizeof(float));
        size_t cb=(size_t)c->kda_proj*c->conv_k*sizeof(float);
        if(m->cwq[i]) memset(m->cwq[i],0,cb);
        if(m->cwk[i]) memset(m->cwk[i],0,cb);
        if(m->cwv[i]) memset(m->cwv[i],0,cb);
    }
}

static void kda_forward(Model *m, Layer *l, int li, const float *x, int C, float *out){
    Cfg *c=&m->c;
    int P=c->kda_proj, H=c->kda_heads, hd=c->kda_hd, K=c->conv_k, D=c->hidden, R=c->kda_rank;
    float *q=falloc((int64_t)C*P), *k=falloc((int64_t)C*P), *v=falloc((int64_t)C*P);
    float *gp=falloc((int64_t)C*P), *on=falloc((int64_t)C*P);
    float *t1=falloc((int64_t)C*R), *graw=falloc((int64_t)C*P), *braw=falloc((int64_t)C*H);

    matmul_qt(q,x,&l->kq,C); matmul_qt(k,x,&l->kk,C); matmul_qt(v,x,&l->kv,C);
    /* decay LoRA and -- GLM-5.3 only -- the gate through the same rank. Kimi computes
     * the gate with one dense w_matmul; the two-step here is the whole difference. */
    f32_matmul(t1, x, l->fa, C, D, R);   f32_matmul(graw, t1, l->fb, C, R, P);
    f32_matmul(t1, x, l->ga, C, D, R);   f32_matmul(gp,   t1, l->gb, C, R, P);
    f32_matmul(braw, x, l->bp, C, D, H);

    float qscale=1.f/sqrtf((float)hd);
    for(int t=0;t<C;t++){
        float *qt_=q+(int64_t)t*P, *kt=k+(int64_t)t*P, *tv=v+(int64_t)t*P;
        float *gpt=gp+(int64_t)t*P, *ont=on+(int64_t)t*P;
        const float *rgt=graw+(int64_t)t*P, *bt=braw+(int64_t)t*H;
        /* stateful depthwise causal conv + SiLU on q, k and v (window persists across tokens) */
        {
            float *wins[3]={m->cwq[li],m->cwk[li],m->cwv[li]};
            float *vecs[3]={qt_,kt,tv};
            const float *taps[3]={l->conv_q,l->conv_k,l->conv_v};
        ...
        #pragma omp parallel for schedule(static)
        for(int h=0;h<H;h++){
            const float *qh=qt_+(int64_t)h*hd, *kh=kt+(int64_t)h*hd, *vh=tv+(int64_t)h*hd;
            ...
            for(int i=0;i<hd;i++){
                float z=rgt[(int64_t)h*hd+i]+l->dt[(int64_t)h*hd+i];
                alpha[i]=expf(c->gate_lb*sigmoidf(l->A[h]*z));
            }
            float beta=sigmoidf(bt[h]);
            float *S_=m->kstate[li]+(int64_t)h*hd*hd;
            memset(kS,0,sizeof(kS));
            for(int kk=0;kk<hd;kk++){
                float *row=S_+(int64_t)kk*hd; float al=alpha[kk], kv_=kn[kk];
                for(int vv=0;vv<hd;vv++){ row[vv]*=al; kS[vv]+=kv_*row[vv]; }
            }
            for(int vv=0;vv<hd;vv++) vt[vv]=(vh[vv]-kS[vv])*beta;
            memset(oh,0,sizeof(oh));
            for(int kk=0;kk<hd;kk++){
                float *row=S_+(int64_t)kk*hd; float kv_=kn[kk], qq=qn[kk];
                for(int vv=0;vv<hd;vv++){ row[vv]+=kv_*vt[vv]; oh[vv]+=qq*row[vv]; }
            }
            /* per-head RMSNorm, then the sigmoid output gate */
            double ms=0; for(int vv=0;vv<hd;vv++) ms+=(double)oh[vv]*oh[vv];
            float r=1.f/sqrtf((float)(ms/hd)+c->eps);
            float *dst=ont+(int64_t)h*hd;
            for(int vv=0;vv<hd;vv++) dst[vv]=oh[vv]*r*l->onw[vv]*sigmoidf(gpt[(int64_t)h*hd+vv]);
        }
    }
    matmul_qt(out,on,&l->ko,C);
...
#endif
    /* ---------------- GLM-5.3 mHC residual ----------------
     * With mHC the residual is [S,M,D] and each of the two sites runs:
     *   post,comb,collapsed = hc(streams);  y = block(rmsnorm(collapsed));
     *   streams = post (x) y  +  comb^T @ streams
     * Both the order and the comb TRANSPOSE follow the reference implementations
     * (DeepseekV4DecoderLayer.forward, and llama.cpp build_hc_pre/build_hc_post):
     * Sinkhorn yields a doubly-stochastic but NON-symmetric matrix, so contracting
     * the wrong axis is silently wrong rather than merely inaccurate. dsv4_mhc_post
     * already sums over the FIRST axis, which is the transposed form. */
    /* hc_attn_fn NULL => this layer has no hyper-connections (the MTP block), so its
     * residual is a single [S,D] stream. Branching on c->hc_mult alone sent the MTP head
     * down the mHC path with NULL weights and a buffer 1/M the size the path assumes. */
    if(c->hc_mult>1 && l->hc_attn_fn){
        int M=c->hc_mult;
        float *post=falloc((int64_t)S*M), *comb=falloc((int64_t)S*M*M);
        float *coll=falloc((int64_t)S*D), *scr=falloc((int64_t)M*D);
        /* --- attention site --- */
        for(int sx=0;sx<S;sx++)
            mhc_block_pre(c, x+(int64_t)sx*M*D, l->hc_attn_fn, l->hc_attn_scale,
                          l->hc_attn_base, post+(int64_t)sx*M, comb+(int64_t)sx*M*M,
                          coll+(int64_t)sx*D);
        ...
        for(int sx=0;sx<S;sx++){
            mhc_block_post(c, tmp+(int64_t)sx*D, x+(int64_t)sx*M*D,
                           post+(int64_t)sx*M, comb+(int64_t)sx*M*M, scr);
            memcpy(x+(int64_t)sx*M*D, scr, (size_t)M*D*sizeof(float));
        }
        /* --- FFN site --- */
        for(int sx=0;sx<S;sx++)
            mhc_block_pre(c, x+(int64_t)sx*M*D, l->hc_ffn_fn, l->hc_ffn_scale,
                          l->hc_ffn_base, post+(int64_t)sx*M, comb+(int64_t)sx*M*M,
                          coll+(int64_t)sx*D);
        for(int sx=0;sx<S;sx++) rmsnorm(nrm+(int64_t)sx*D, coll+(int64_t)sx*D, l->post_ln, D, c->eps);
        ...
        free(post); free(comb); free(coll); free(scr);
        return;
    }
    for(int s=0;s<S;s++) rmsnorm(nrm+(int64_t)s*D, x+(int64_t)s*D, l->in_ln, D, c->eps);
    /* GLM-5.3: 34 of 45 layers are KDA. The recurrence carries its own state and ignores
     * pos_base/kvs entirely -- position lives in the decay, not in a positional encoding.
...
/* ---------------- GLM-5.3 KDA self-test (GLM53_KDA_SELFTEST=<fixture>) ----------------
 * Drives the real kda_forward() against work/kda_oracle.py, which is adapted from
 * colibri's own numpy KDA reference (tools/k3_ref.py, itself derived from Moonshot's
 * modeling code) with GLM-5.3's low-rank g_a/g_b gate substituted for Kimi's dense
 * g_proj. Weights are f32 (QT fmt=0) so the comparison isolates the recurrence from
 * any quantization error.
 */
static float *kt_rd(FILE *f, int64_t n){
    float *p=falloc(n);
    if(fread(p,sizeof(float),(size_t)n,f)!=(size_t)n){ fprintf(stderr,"kda selftest: short read %lld\n",(long long)n); exit(1); }
    return p;
}
static QT kt_qt(float *w,int O,int I){ QT t; memset(&t,0,sizeof t); t.fmt=0; t.qf=w; t.O=O; t.I=I; return t; }

static int glm53_kda_selftest(const char *path){
    FILE *f=fopen(path,"rb"); if(!f){ perror(path); return 1; }
    int32_t H,hd,D,K,T; float lb,eps;
    if(fread(&H,4,1,f)!=1||fread(&hd,4,1,f)!=1||fread(&D,4,1,f)!=1||
       fread(&K,4,1,f)!=1||fread(&T,4,1,f)!=1||fread(&lb,4,1,f)!=1||fread(&eps,4,1,f)!=1){
        fprintf(stderr,"kda selftest: bad header\n"); return 1; }
    int Pp=H*hd, R=hd;
    printf("kda fixture: H=%d hd=%d D=%d conv_k=%d T=%d gate_lb=%g eps=%g\n",H,hd,D,K,T,lb,eps);

    Model *m=calloc(1,sizeof(Model)); Layer *l=calloc(1,sizeof(Layer));
    Cfg *c=&m->c;
    c->hidden=D; c->kda_heads=H; c->kda_hd=hd; c->kda_proj=Pp; c->kda_rank=R;
    c->conv_k=K; c->gate_lb=lb; c->eps=eps; c->n_layers=1; c->is_kda[0]=1;

    float *wq=kt_rd(f,(int64_t)Pp*D), *wk=kt_rd(f,(int64_t)Pp*D);
    float *wv=kt_rd(f,(int64_t)Pp*D), *wo=kt_rd(f,(int64_t)D*Pp);
    l->kq=kt_qt(wq,Pp,D); l->kk=kt_qt(wk,Pp,D); l->kv=kt_qt(wv,Pp,D); l->ko=kt_qt(wo,D,Pp);
    l->conv_q=kt_rd(f,(int64_t)Pp*K); l->conv_k=kt_rd(f,(int64_t)Pp*K); l->conv_v=kt_rd(f,(int64_t)Pp*K);
    l->fa=kt_rd(f,(int64_t)R*D); l->fb=kt_rd(f,(int64_t)Pp*R);
    l->ga=kt_rd(f,(int64_t)R*D); l->gb=kt_rd(f,(int64_t)Pp*R);
    l->bp=kt_rd(f,(int64_t)H*D); l->dt=kt_rd(f,Pp);
    l->A=kt_rd(f,H); for(int h=0;h<H;h++) l->A[h]=expf(l->A[h]);   /* engine applies exp at load */
    l->onw=kt_rd(f,hd);
    float *x=kt_rd(f,(int64_t)T*D), *want=kt_rd(f,(int64_t)T*D);
    fclose(f);

    m->kstate=calloc(1,sizeof(float*)); m->cwq=calloc(1,sizeof(float*));
    m->cwk=calloc(1,sizeof(float*));    m->cwv=calloc(1,sizeof(float*));
    m->kstate[0]=calloc((size_t)H*hd*hd,sizeof(float));
    m->cwq[0]=calloc((size_t)Pp*K,sizeof(float));
    m->cwk[0]=calloc((size_t)Pp*K,sizeof(float));
    m->cwv[0]=calloc((size_t)Pp*K,sizeof(float));

    float *got=falloc((int64_t)T*D);
    kda_forward(m,l,0,x,T,got);

    double maxe=0,sum=0,ref=0; int at=0;
    for(int64_t i=0;i<(int64_t)T*D;i++){
        double e=fabs((double)got[i]-(double)want[i]);
        sum+=e; ref+=fabs((double)want[i]);
        if(e>maxe){ maxe=e; at=(int)i; }
    }
    double mean=sum/((double)T*D), rel=sum/(ref>0?ref:1);
    printf("[KDA batch vs numpy oracle]  max|err|=%.3e  mean|err|=%.3e  rel=%.3e\n",maxe,mean,rel);
    printf("  worst at [%d]: got %.9g want %.9g\n",at,got[at],want[at]);
    int ok = rel < 1e-5;

    /* Incremental decode: same tokens, one call each, from a reset state. The KDA
     * recurrence and the conv windows persist across calls, so this must reproduce the
     * batch result exactly -- it is the property the decode path relies on, and a
     * mis-advanced conv window survives the batch test but not this one. */
    memset(m->kstate[0],0,(size_t)H*hd*hd*sizeof(float));
    memset(m->cwq[0],0,(size_t)Pp*K*sizeof(float));
    memset(m->cwk[0],0,(size_t)Pp*K*sizeof(float));
    memset(m->cwv[0],0,(size_t)Pp*K*sizeof(float));
    float *inc=falloc((int64_t)T*D);
    for(int t=0;t<T;t++) kda_forward(m,l,0,x+(int64_t)t*D,1,inc+(int64_t)t*D);
    double imax=0,isum=0; int iat=0;
    for(int64_t i=0;i<(int64_t)T*D;i++){
        double e=fabs((double)inc[i]-(double)got[i]);
        isum+=e; if(e>imax){ imax=e; iat=(int)i; }
    }
    double irel=isum/(ref>0?ref:1);
    printf("[KDA incremental vs batch]   max|err|=%.3e  mean|err|=%.3e  rel=%.3e\n",
           imax, isum/((double)T*D), irel);
    if(imax>0) printf("  worst at [%d]: incr %.9g batch %.9g\n",iat,inc[iat],got[iat]);
    int ok2 = irel < 1e-6;
    if(!ok2) printf("  -> stateful conv/recurrence does NOT carry across calls\n");
...
/* ------------- GLM-5.3 whole-layer self-test (GLM53_LAYER_SELFTEST=<fixture>) ------------- */
static int glm53_layer_selftest(const char *path){
    FILE *f=fopen(path,"rb"); if(!f){ perror(path); return 1; }
    int32_t H,hd,D,K,T,M,I,iters; float lb,eps,hceps,slim;
    int32_t hdr[8]; float fhdr[4];
    if(fread(hdr,4,8,f)!=8 || fread(fhdr,4,4,f)!=4){ fprintf(stderr,"layer selftest: bad header\n"); return 1; }
    H=hdr[0]; hd=hdr[1]; D=hdr[2]; K=hdr[3]; T=hdr[4]; M=hdr[5]; I=hdr[6]; iters=hdr[7];
    lb=fhdr[0]; eps=fhdr[1]; hceps=fhdr[2]; slim=fhdr[3];
    int Pp=H*hd, R=hd, N=2*M+M*M;
    printf("layer fixture: H=%d hd=%d D=%d K=%d T=%d hc_mult=%d dense_inter=%d iters=%d\n",H,hd,D,K,T,M,I,iters);
    printf("               gate_lb=%g eps=%g hc_eps=%g swiglu_limit=%g\n",lb,eps,hceps,slim);

    Model *m=calloc(1,sizeof(Model)); Layer *l=calloc(1,sizeof(Layer));
    Cfg *c=&m->c;
    c->hidden=D; c->kda_heads=H; c->kda_hd=hd; c->kda_proj=Pp; c->kda_rank=R;
    c->conv_k=K; c->gate_lb=lb; c->eps=eps; c->n_layers=1; c->is_kda[0]=1;
    c->hc_mult=M; c->hc_eps=hceps; c->hc_sinkhorn_iters=iters;
    c->dense_inter=I; c->first_dense=1; c->swiglu_limit=slim; g_swiglu_limit=slim;
    m->L=l;

    l->in_ln=kt_rd(f,D); l->post_ln=kt_rd(f,D);
    l->hc_attn_fn=kt_rd(f,(int64_t)N*M*D); l->hc_attn_base=kt_rd(f,N); l->hc_attn_scale=kt_rd(f,3);
    l->hc_ffn_fn =kt_rd(f,(int64_t)N*M*D); l->hc_ffn_base =kt_rd(f,N); l->hc_ffn_scale =kt_rd(f,3);
    float *wq=kt_rd(f,(int64_t)Pp*D), *wk=kt_rd(f,(int64_t)Pp*D);
    float *wv=kt_rd(f,(int64_t)Pp*D), *wo=kt_rd(f,(int64_t)D*Pp);
    l->kq=kt_qt(wq,Pp,D); l->kk=kt_qt(wk,Pp,D); l->kv=kt_qt(wv,Pp,D); l->ko=kt_qt(wo,D,Pp);
    l->conv_q=kt_rd(f,(int64_t)Pp*K); l->conv_k=kt_rd(f,(int64_t)Pp*K); l->conv_v=kt_rd(f,(int64_t)Pp*K);
    ...
    l->ga=kt_rd(f,(int64_t)R*D); l->gb=kt_rd(f,(int64_t)Pp*R);
    l->bp=kt_rd(f,(int64_t)H*D); l->dt=kt_rd(f,Pp);
    l->A=kt_rd(f,H); for(int h=0;h<H;h++) l->A[h]=expf(l->A[h]);
    l->onw=kt_rd(f,hd);
    float *wg=kt_rd(f,(int64_t)I*D), *wu=kt_rd(f,(int64_t)I*D), *wd=kt_rd(f,(int64_t)D*I);
    l->gate_proj=kt_qt(wg,I,D); l->up_proj=kt_qt(wu,I,D); l->down_proj=kt_qt(wd,D,I);
    l->sparse=0;
    float *x=kt_rd(f,(int64_t)T*M*D), *want=kt_rd(f,(int64_t)T*M*D);
    fclose(f);
    ...
    float *nrm=falloc((int64_t)T*D), *tmp=falloc((int64_t)T*D);
    layer_forward_rows(m,l,0,x,T,0,NULL,NULL,nrm,tmp);   /* x updated in place: [T,M,D] */

    double maxe=0,sum=0,ref=0; int at=0;
    for(int64_t i=0;i<(int64_t)T*M*D;i++){
        double e=fabs((double)x[i]-(double)want[i]);
        sum+=e; ref+=fabs((double)want[i]);
        if(e>maxe){ maxe=e; at=(int)i; }
    }
    double rel=sum/(ref>0?ref:1);
    double peak=0.0;
    for(int64_t i=0;i<(int64_t)T*M*D;i++){ double a=fabs((double)want[i]); if(a>peak) peak=a; }
    double relmax = maxe/(peak>0?peak:1);
    printf("[whole layer vs numpy oracle]  max|err|=%.3e  mean|err|=%.3e  mean-rel=%.3e\n",
           maxe, sum/((double)T*M*D), rel);
    printf("  worst at [%d]: got %.9g want %.9g   max-rel=%.3e\n",at,x[at],want[at],relmax);
    /* Gate on the WORST element, not the mean: a mean-relative gate averages a localized
     * fault away (a single element off by 1e-2 moved mean-rel to only 7e-5). Both mHC
     * sites round to bf16, so a real disagreement is >= one bf16 ulp (~4e-3 relative);
     * 1e-4 sits well below that and well above an exact match. */
    /* Outputs are bf16-rounded at both mHC sites, so one bf16 ulp (~2^-8 = 3.9e-3) is the
     * finest difference the model can itself represent. At small dims engine and oracle
     * agree bit-for-bit in f32 and this reads 0; at production dims their summation orders
     * differ in the last bits, so a value near a bf16 boundary occasionally rounds the other
     * way. Gate at one ulp but PRINT max-rel, so a regression under the gate stays visible.
     */
    int ok = relmax < 5e-3;
    printf("\n%s\n", ok?"PASS":"FAIL");
    return ok?0:1;
...
/* ---------- GLM-5.3 sparse-MoE layer self-test (GLM53_MOE_SELFTEST=<fixture>) ---------- */
static int glm53_moe_selftest(const char *path){
    FILE *f=fopen(path,"rb"); if(!f){ perror(path); return 1; }
    int32_t hdr[12]; float fh[5];
    if(fread(hdr,4,12,f)!=12 || fread(fh,4,5,f)!=5){ fprintf(stderr,"moe selftest: bad header\n"); return 1; }
    int H=hdr[0],hd=hdr[1],D=hdr[2],K=hdr[3],T=hdr[4],M=hdr[5],iters=hdr[6];
    int E=hdr[7],topk=hdr[8],MI=hdr[9],nsh=hdr[10],ntk=hdr[11];
    float lb=fh[0],eps=fh[1],hceps=fh[2],slim=fh[3],rscale=fh[4];
    int Pp=H*hd, R=hd, N=2*M+M*M, sI=MI*nsh;
    printf("moe fixture: H=%d hd=%d D=%d T=%d hc_mult=%d | E=%d topk=%d moe_inter=%d n_shared=%d\n",
           H,hd,D,T,M,E,topk,MI,nsh);
    printf("             norm_topk=%d routed_scale=%g swiglu_limit=%g\n",ntk,rscale,slim);
    ...
    c->conv_k=K; c->gate_lb=lb; c->eps=eps; c->n_layers=1; c->is_kda[0]=1;
    c->hc_mult=M; c->hc_eps=hceps; c->hc_sinkhorn_iters=iters;
    c->n_experts=E; c->topk=topk; c->moe_inter=MI; c->n_shared=nsh;
    c->norm_topk=ntk; c->routed_scale=rscale; c->first_dense=0;
    c->swiglu_limit=slim; g_swiglu_limit=slim;
    m->L=l;
    ...
    l->A=kt_rd(f,H); for(int h=0;h<H;h++) l->A[h]=expf(l->A[h]);
    l->onw=kt_rd(f,hd);
    l->router=kt_rd(f,(int64_t)E*D); l->router_bias=kt_rd(f,E);
    float *sg=kt_rd(f,(int64_t)sI*D), *su=kt_rd(f,(int64_t)sI*D), *sd=kt_rd(f,(int64_t)D*sI);
    l->sh_gate=kt_qt(sg,sI,D); l->sh_up=kt_qt(su,sI,D); l->sh_down=kt_qt(sd,D,sI);
    l->sparse=1;

    /* Pin every routed expert: moe() checks pin_indexed() before the LRU and before
     * any disk path, so a fully pinned layer never reaches the expert store. */
    m->pin=calloc(1,sizeof(ESlot*)); m->npin=calloc(1,sizeof(int));
    m->pin_slot_by_expert=calloc(1,sizeof(int*));
    m->pin[0]=calloc((size_t)E,sizeof(ESlot));
    m->pin_slot_by_expert[0]=calloc((size_t)E,sizeof(int));
    for(int e=0;e<E;e++){
        float *g=kt_rd(f,(int64_t)MI*D), *u=kt_rd(f,(int64_t)MI*D), *d=kt_rd(f,(int64_t)D*MI);
        m->pin[0][e].eid=e;
        m->pin[0][e].g=kt_qt(g,MI,D); m->pin[0][e].u=kt_qt(u,MI,D); m->pin[0][e].d=kt_qt(d,D,MI);
        m->pin_slot_by_expert[0][e]=e;
    }
    m->npin[0]=E;
    ...
    /* per-layer bookkeeping moe() writes into unconditionally */
    m->eusage=calloc(1,sizeof(uint32_t*)); m->eheat=calloc(1,sizeof(uint32_t*));
    m->elast=calloc(1,sizeof(uint32_t*));  m->elast_dc=calloc(1,sizeof(uint32_t*));
    m->elast_pre=calloc(1,sizeof(uint32_t*));
    m->eusage[0]=calloc((size_t)E,sizeof(uint32_t)); m->eheat[0]=calloc((size_t)E,sizeof(uint32_t));
    m->elast[0]=calloc((size_t)E,sizeof(uint32_t));  m->elast_dc[0]=calloc((size_t)E,sizeof(uint32_t));
    m->elast_pre[0]=calloc((size_t)E,sizeof(uint32_t));
    m->ecache=calloc(1,sizeof(ESlot*)); m->ecn=calloc(1,sizeof(int));
    m->ecache_slot_by_expert=calloc(1,sizeof(int*));
    m->ecache_slot_by_expert[0]=calloc((size_t)E,sizeof(int));
    for(int e=0;e<E;e++) m->ecache_slot_by_expert[0][e]=-1;
    m->eroute=calloc(1,sizeof(int*)); m->enr=calloc(1,sizeof(int));
    m->eroute[0]=calloc((size_t)topk,sizeof(int));
    ...
    float *nrm=falloc((int64_t)T*D), *tmp=falloc((int64_t)T*D);
    layer_forward_rows(m,l,0,x,T,0,NULL,NULL,nrm,tmp);

    double maxe=0,sum=0,peak=0; int at=0;
    for(int64_t i=0;i<(int64_t)T*M*D;i++){
        double e=fabs((double)x[i]-(double)want[i]);
        sum+=e; if(e>maxe){ maxe=e; at=(int)i; }
        double a=fabs((double)want[i]); if(a>peak) peak=a;
    }
    double relmax=maxe/(peak>0?peak:1);
    printf("[sparse MoE layer vs numpy oracle]  max|err|=%.3e  mean|err|=%.3e  max-rel=%.3e\n",
           maxe, sum/((double)T*M*D), relmax);
    printf("  worst at [%d]: got %.9g want %.9g\n",at,x[at],want[at]);
    /* Outputs are bf16-rounded at both mHC sites, so one bf16 ulp (~2^-8 = 3.9e-3) is the
     * finest difference the model can itself represent. At small dims engine and oracle
     * agree bit-for-bit in f32 and this reads 0; at production dims their summation orders
...
static void kv_alloc(Model *m, int max_t);   /* defined below; the MLA self-test needs the KV cache */
/* ---------- GLM-5.3 MLA/DSA layer self-test (GLM53_MLA_SELFTEST=<fixture>) ---------- */
static int glm53_mla_selftest(const char *path){
    FILE *f=fopen(path,"rb"); if(!f){ perror(path); return 1; }
    int32_t hdr[10]; float fh[4];
    if(fread(hdr,4,10,f)!=10 || fread(fh,4,4,f)!=4){ fprintf(stderr,"mla selftest: bad header\n"); return 1; }
    int D=hdr[0],T=hdr[1],M=hdr[2],iters=hdr[3],NH=hdr[4],QL=hdr[5],KL=hdr[6],NOPE=hdr[7],VH=hdr[8],I=hdr[9];
    float eps=fh[0],hceps=fh[1],slim=fh[2],ascale=fh[3];
    int N=2*M+M*M, QH=NOPE;                       /* qk_rope_head_dim = 0 under NoPE */
    printf("mla fixture: D=%d T=%d hc_mult=%d | heads=%d q_lora=%d kv_lora=%d nope=%d v_head=%d\n",
           D,T,M,NH,QL,KL,NOPE,VH);
    printf("             attn_scale=%g (1/sqrt(%d)) dense_inter=%d swiglu_limit=%g\n",ascale,QH,I,slim);

    Model *m=calloc(1,sizeof(Model)); Layer *l=calloc(1,sizeof(Layer));
    Cfg *c=&m->c;
    c->hidden=D; c->n_layers=1; c->is_kda[0]=0; c->n_heads=NH;
    c->q_lora=QL; c->kv_lora=KL; c->qk_nope=NOPE; c->qk_rope=0; c->v_head=VH;
    c->qk_head=QH; c->attn_scale=ascale; c->mla_nope=1;
    c->hc_mult=M; c->hc_eps=hceps; c->hc_sinkhorn_iters=iters; c->eps=eps;
    c->dense_inter=I; c->first_dense=1; c->swiglu_limit=slim; g_swiglu_limit=slim;
    c->idx_type[0]=0; m->has_dsa=0; m->L=l;

    l->in_ln=kt_rd(f,D); l->post_ln=kt_rd(f,D);
    l->hc_attn_fn=kt_rd(f,(int64_t)N*M*D); l->hc_attn_base=kt_rd(f,N); l->hc_attn_scale=kt_rd(f,3);
    l->hc_ffn_fn =kt_rd(f,(int64_t)N*M*D); l->hc_ffn_base =kt_rd(f,N); l->hc_ffn_scale =kt_rd(f,3);
    float *qa=kt_rd(f,(int64_t)QL*D);   l->q_a =kt_qt(qa,QL,D);
    l->q_a_ln=kt_rd(f,QL);
    float *qb=kt_rd(f,(int64_t)NH*QH*QL); l->q_b=kt_qt(qb,NH*QH,QL);
    float *ka=kt_rd(f,(int64_t)KL*D);   l->kv_a=kt_qt(ka,KL,D);
    l->kv_a_ln=kt_rd(f,KL);
    float *kb=kt_rd(f,(int64_t)NH*(NOPE+VH)*KL); l->kv_b=kt_qt(kb,NH*(NOPE+VH),KL);
    float *wo=kt_rd(f,(int64_t)D*NH*VH); l->o=kt_qt(wo,D,NH*VH);
    float *wg=kt_rd(f,(int64_t)I*D), *wu=kt_rd(f,(int64_t)I*D), *wd=kt_rd(f,(int64_t)D*I);
    l->gate_proj=kt_qt(wg,I,D); l->up_proj=kt_qt(wu,I,D); l->down_proj=kt_qt(wd,D,I);
    l->sparse=0;
    float *x=kt_rd(f,(int64_t)T*M*D), *want=kt_rd(f,(int64_t)T*M*D);
    ...
    m->kv=calloc(1,sizeof(KVState));
    m->kv_start=m->kv->kv_start=calloc(2,sizeof(int));
    kv_alloc(m,T);

    /* main() reads ABSORB after this hook returns, so honour it here. The absorb
     * path attends in latent space; the explicit path materialises K/V. They must agree. */
    if(getenv("ABSORB")) g_absorb=atoi(getenv("ABSORB"));
    printf("             absorb=%d (auto -1 => absorb when S<=4; S=%d)\n", g_absorb, T);
    float *nrm=falloc((int64_t)T*D), *tmp=falloc((int64_t)T*D);
    layer_forward_rows(m,l,0,x,T,0,NULL,NULL,nrm,tmp);
    ...
    for(int t=0;t<T;t++){
        double rm=0;
        for(int64_t z=(int64_t)t*M*D;z<(int64_t)(t+1)*M*D;z++){
            double e=fabs((double)x[z]-(double)want[z]); if(e>rm) rm=e; }
        printf("    row %d max|err| = %.3e\n", t, rm);
    }
    double relmax=maxe/(peak>0?peak:1);
    printf("[MLA layer vs transformers oracle]  max|err|=%.3e  mean|err|=%.3e  max-rel=%.3e\n",
           maxe, sum/((double)T*M*D), relmax);
    printf("  worst at [%d]: got %.9g want %.9g\n",at,x[at],want[at]);
    /* The oracle is fp64 torch, the engine fp32 with bf16 rounding at both mHC sites,
     * so ~2^-8 relative is the floor -- unlike the KDA/MoE tests whose oracles round
     * to bf16 themselves. Gate a little above one bf16 ulp. */
    int ok = relmax < 1e-2;
    printf("\n%s\n", ok?"PASS":"FAIL");
    return ok?0:1;
}
...
/* ---------- GLM-5.3 quantized MoE layer self-test (GLM53_MOEQ_SELFTEST) ---------- */
static QT kt_qt8(FILE *f,int O,int I){                     /* fmt=1: int8 + per-row scale */
    QT t; memset(&t,0,sizeof t); t.fmt=1; t.O=O; t.I=I;
    t.q8=(int8_t*)qalloc((size_t)O*I);
    if(fread(t.q8,1,(size_t)O*I,f)!=(size_t)O*I){ fprintf(stderr,"q8 short read\n"); exit(1); }
    t.s=(float*)qalloc((size_t)O*sizeof(float));
    if(fread(t.s,sizeof(float),(size_t)O,f)!=(size_t)O){ fprintf(stderr,"q8 scale short read\n"); exit(1); }
    return t;
}
static QT kt_qt4g(FILE *f,int O,int I,int gs){             /* fmt=4: int4 + per-group scale */
    QT t; memset(&t,0,sizeof t); t.fmt=4; t.O=O; t.I=I; t.gs=gs;
    size_t rb=(size_t)((I+1)/2), ng=(size_t)((I+gs-1)/gs);
    t.q4=(uint8_t*)qalloc((size_t)O*rb);
    if(fread(t.q4,1,(size_t)O*rb,f)!=(size_t)O*rb){ fprintf(stderr,"q4 short read\n"); exit(1); }
    t.s=(float*)qalloc((size_t)O*ng*sizeof(float));
    if(fread(t.s,sizeof(float),(size_t)O*ng,f)!=(size_t)O*ng){ fprintf(stderr,"q4 scale short read\n"); exit(1); }
    return t;
}

static int glm53_moeq_selftest(const char *path){
    FILE *f=fopen(path,"rb"); if(!f){ perror(path); return 1; }
    int32_t hdr[13]; float fh[5];
    if(fread(hdr,4,13,f)!=13 || fread(fh,4,5,f)!=5){ fprintf(stderr,"moeq: bad header\n"); return 1; }
    int H=hdr[0],hd=hdr[1],D=hdr[2],K=hdr[3],T=hdr[4],M=hdr[5],iters=hdr[6];
    int E=hdr[7],topk=hdr[8],MI=hdr[9],nsh=hdr[10],ntk=hdr[11],gs=hdr[12];
    float lb=fh[0],eps=fh[1],hceps=fh[2],slim=fh[3],rscale=fh[4];
    int Pp=H*hd,R=hd,N=2*M+M*M,sI=MI*nsh;
    printf("moeq fixture: D=%d T=%d E=%d topk=%d moe_inter=%d | experts int4-gs%d, resident int8\n",
           D,T,E,topk,MI,gs);
    ...
    c->n_experts=E; c->topk=topk; c->moe_inter=MI; c->n_shared=nsh;
    c->norm_topk=ntk; c->routed_scale=rscale; c->first_dense=0;
    c->swiglu_limit=slim; g_swiglu_limit=slim; m->L=l;

    l->in_ln=kt_rd(f,D); l->post_ln=kt_rd(f,D);
    l->hc_attn_fn=kt_rd(f,(int64_t)N*M*D); l->hc_attn_base=kt_rd(f,N); l->hc_attn_scale=kt_rd(f,3);
    l->hc_ffn_fn =kt_rd(f,(int64_t)N*M*D); l->hc_ffn_base =kt_rd(f,N); l->hc_ffn_scale =kt_rd(f,3);
    l->kq=kt_qt8(f,Pp,D); l->kk=kt_qt8(f,Pp,D); l->kv=kt_qt8(f,Pp,D); l->ko=kt_qt8(f,D,Pp);
    l->conv_q=kt_rd(f,(int64_t)Pp*K); l->conv_k=kt_rd(f,(int64_t)Pp*K); l->conv_v=kt_rd(f,(int64_t)Pp*K);
    l->fa=kt_rd(f,(int64_t)R*D); l->fb=kt_rd(f,(int64_t)Pp*R);
    l->ga=kt_rd(f,(int64_t)R*D); l->gb=kt_rd(f,(int64_t)Pp*R);
    l->bp=kt_rd(f,(int64_t)H*D); l->dt=kt_rd(f,Pp);
    ...
    l->onw=kt_rd(f,hd);
    l->router=kt_rd(f,(int64_t)E*D); l->router_bias=kt_rd(f,E);
    l->sh_gate=kt_qt8(f,sI,D); l->sh_up=kt_qt8(f,sI,D); l->sh_down=kt_qt8(f,D,sI);
    l->sparse=1;
    ...
    for(int e=0;e<E;e++){
        m->pin[0][e].eid=e;
        m->pin[0][e].g=kt_qt4g(f,MI,D,gs);
        m->pin[0][e].u=kt_qt4g(f,MI,D,gs);
        m->pin[0][e].d=kt_qt4g(f,D,MI,gs);
        m->pin_slot_by_expert[0][e]=e;
    }
    m->npin[0]=E;
    ...
    /* main() reads these after the hook returns; honour them here. IDOT=0 = exact f32
     * kernels (dequantize then f32 matmul); IDOT=1 = int4 x int8 with quantized
     * activations, which is the shipping path. */
    if(getenv("IDOT")) g_idot=atoi(getenv("IDOT"));
    printf("             IDOT=%d (%s)\n", g_idot,
           g_idot? "int4xint8, activations quantized (W4A8)" : "exact f32 kernels (W4A16)");
    float *nrm=falloc((int64_t)T*D), *tmp=falloc((int64_t)T*D);
    layer_forward_rows(m,l,0,x,T,0,NULL,NULL,nrm,tmp);
    ...
    double relmax=maxe/(peak>0?peak:1);
    printf("[int4-gs%d MoE layer vs dequant oracle]  max|err|=%.3e  mean|err|=%.3e  max-rel=%.3e\n",
           gs, maxe, sum/((double)T*M*D), relmax);
    printf("  worst at [%d]: got %.9g want %.9g\n",at,x[at],want[at]);
    /* Both sides see the SAME dequantized weights, so WEIGHT quantization error cancels.
     * What remains depends on the kernel family -- see the two gates below. */
    double tol = g_idot ? 5e-2 : 1e-4;
    int ok = relmax < tol;
    if(g_idot)
        printf("  (W4A8: the residual is int8 ACTIVATION quantization, which the oracle\n"
               "   does not model. Phase-1 quant_ablation round-trips WEIGHTS only, so it\n"
               "   does not account for this either. tol=%.0e)\n", tol);
    else
        printf("  (W4A16: identical arithmetic to the oracle -- strict gate on int4-gs64\n"
               "   unpacking, group-scale indexing and int8 row scales. tol=%.0e)\n", tol);
    printf("\n%s\n", ok?"PASS":"FAIL");
    return ok?0:1;
}
...
static void layers_forward_rows(Model *m, float *x, int S, int pos_base,
    ...
    /* ---- GLM-5.3 mHC: [S,D] -> [S,M,D] ----
     * Reference seeds the streams by BROADCAST, not by putting the embedding in
     * stream 0 and zeroing the rest: transformers does
     *   inputs_embeds.unsqueeze(2).expand(-1,-1,hc_mult,-1)
     * (DeepseekV4Model.forward) and llama.cpp does the matching repeat_4d after a
     * reshape_3d. Collapse at the end is a plain arithmetic MEAN over the streams --
     * GLM-5.3 diverges from DeepSeek-V4 here: DeepSeek applies a LEARNED collapse
     * (DeepseekV4HyperHead, with its own hc_fn/hc_base/hc_scale), but GLM-5.3 ships
    ...
    float *xs = x;
    if(hcM>1){
        xs = falloc((int64_t)S*hcM*D);
        glm53_hc_seed(x, S, hcM, D, xs);
    }
#ifdef COLI_CUDA
...
#endif
        layer_forward_rows(m,&m->L[i],i,xs,S,pos_base,kvs,positions,nrm,tmp);
    }
#ifdef COLI_CUDA
    if(x_dev_on>=0) coli_cuda_pipe_download(x_dev_on,x_dev,x,xb);
#endif
    free(nrm); free(tmp);
    if(hcM>1){                                   /* collapse: mean over the M streams */
        glm53_hc_mean(xs, S, hcM, D, x);
        free(xs);
    }
}
...
static void kv_alloc(Model *m, int max_t){
    ...
    if(k->Gc){ for(int i=0;i<c->n_layers;i++) free(k->Gc[i]); free(k->Gc); k->Gc=NULL; }
    if(m->has_dsa){
        k->Ic=calloc(c->n_layers,sizeof(float*));
        for(int i=0;i<c->n_layers;i++) if(c->idx_type[i]) k->Ic[i]=falloc((int64_t)max_t*c->index_hd);
        /* k-pool needs the gate logits of every cached token, same cardinality as Ic */
        if(c->index_kpool_compress && c->index_kpool>1){
            k->Gc=calloc(c->n_layers,sizeof(float*));
            for(int i=0;i<c->n_layers;i++) if(c->idx_type[i]) k->Gc[i]=falloc((int64_t)max_t*c->index_hd);
        }
    }
    k->max_t=max_t;
    ...
    m->Lc=k->Lc; m->Rc=k->Rc; m->Ic=k->Ic; m->Gc=k->Gc; m->max_t=k->max_t; m->kv_start=k->kv_start;
...
#endif
    m->kv=k; m->Lc=k->Lc; m->Rc=k->Rc; m->Ic=k->Ic; m->Gc=k->Gc;
    m->Lc8=k->Lc8; m->Rc8=k->Rc8; m->Lsc=k->Lsc; m->Rsc=k->Rsc;
...
static void pin_arena_bind(Model *m, PinRec *r, int *slot_of, int from, int to){
    ...
    for(int l=0;l<NR;l++){
        ...
        for(int k=0;k<3 && ok;k++){
            char nm[288],qn[320];
            snprintf(nm,sizeof nm,"model.%slayers.%d.mlp.experts.%d.%s.weight",g_pfx,l,r[first[l]].e,suf[k]);
            snprintf(qn,sizeof qn,"%s.qs",nm);
...
int main(int argc, char **argv){
    { const char *kt=getenv("GLM53_KDA_SELFTEST");
      if(kt) return glm53_kda_selftest(kt); }
    { const char *lt=getenv("GLM53_LAYER_SELFTEST");
      if(lt) return glm53_layer_selftest(lt); }
    { const char *mt=getenv("GLM53_MOE_SELFTEST");
      if(mt) return glm53_moe_selftest(mt); }
    { const char *at=getenv("GLM53_MLA_SELFTEST");
      if(at) return glm53_mla_selftest(at); }
    { const char *qt=getenv("GLM53_MOEQ_SELFTEST");
      if(qt) return glm53_moeq_selftest(qt); }
     ...
     * claim (#1183). */
    printf("== GLM-5.3 C engine (glm5_next), cache=%d experts/layer | compute experts@%d-bit dense@%d-bit | idot: " IDOT_KERNEL " ==\n", cap, ebits, dbits);
    ...
    { int nk=0; for(int i=0;i<m.c.n_layers && i<128;i++) nk+=m.c.is_kda[i];
      ...
      if(nk && !g_glm53_kda_spec && (g_draft>0 || m.has_mtp)){
          fprintf(stderr,
            ...
            "       on a rejected draft, so speculative decoding would drift. Set\n"
            "       COLI_GLM53_KDA_SPEC=1 to measure the drift anyway (output is WRONG).\n", nk);
          g_draft=0; m.has_mtp=0;
      } }
